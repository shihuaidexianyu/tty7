...
This fork uses [shihuaidexianyu/tty7](https://github.com/shihuaidexianyu/tty7)
for both desktop updates and remote-server downloads. Its release identity is
defined once in `workspace.package.repository` in the root `Cargo.toml`.
The upstream project and dependency sources retain their original attribution.

For the first migration from an upstream binary, install this fork's
[Nightly package](https://github.com/shihuaidexianyu/tty7/releases/tag/nightly)
manually. An already installed upstream binary still contains the upstream
update address. Existing explicit channel preferences are preserved: select
**Nightly** in Settings if your old configuration selected **Stable**.
...
| | |
|---|---|
| **Stable** | Published stable releases in this fork. Reads `/releases/latest`, which excludes prereleases; unavailable until this fork publishes a stable release. |
| **Nightly** *(fork default)* | Rebuilt from this fork's `main` every night. Reads the rolling `nightly` tag. |
